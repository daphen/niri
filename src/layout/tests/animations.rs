...
fn check_ops_with_options(
    ...
    for op in ops {
        if matches!(op, Op::FocusColumnLeft) {
            layout.refresh(true);
            layout.activate_window(&1);
        } else {
            if matches!(op, Op::ConsumeWindowIntoColumn | Op::CompleteAnimations) {
                layout.refresh(true);
            }
            op.apply(&mut layout);
        }
